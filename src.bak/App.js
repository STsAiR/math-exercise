    ...
    >
      ...
      {selectedGymId && (
        <section style={{ marginTop: 40 }}>
          {/* show gym name instead of gym id */}
          {/* Find the gym by selectedGymId */}
          {(() => {
            const gym = gyms.find((g) => g.id === selectedGymId);
            if (!gym) return <h3>Loading reviews...</h3>;
            return (
              <h3>
                Reviews for {gym.name} in {gym.location}
              </h3>
            );
          })()}
          {/* <h3>Reviews for Gym ID {selectedGymId}</h3> */}
          {/* <h3>Reviews for {gymsList[selectedGymId].name} in {gymsList[selectedGymId].location}</h3> */}
